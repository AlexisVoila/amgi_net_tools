...
void socks5_stream_manager::on_error(sys::error_code ec, server_stream_ptr stream)
{
    if (auto it = states_.find(stream->id()); it != states_.end())
        it->second.handle_server_error(ec);
}

void socks5_stream_manager::on_error(sys::error_code ec, client_stream_ptr stream)
{
    if (auto it = states_.find(stream->id()); it != states_.end())
